...
//  }
  ...
  "Auth integrated" - {
    ...
    val app = MockApp(new AuthController(new AuthService(new Crud[Auth, String] {}, client)))

    "wrong provider" in {
      val result = app.get("/signup/facebook")
      result.code should be(307)
      val location: String = result.getHeader("Location")
      val State = ".*state=([a-z0-9A-Z=|_]+).*".r
      val state: String = location match {
        case State(stateParam) => stateParam
        case _ => {
      ...
      val result2 = app.get(s"/auth/fb?code=1234&state=$state")
      result2.code should be (401)
    }
  }
